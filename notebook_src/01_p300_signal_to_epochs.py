...
#
# For event onset sample $s_i$, epoch $X_i[c,k]=x_c[s_i+k]$ uses a relative time coordinate $t_k=k/f_s$. For class $q$, $\bar X_q[c,k]=N_q^{-1}\sum_{i:y_i=q}X_i[c,k]$. If independent zero-mean trial noise has variance $\sigma^2$, the average noise variance is $\sigma^2/N_q$; correlated noise does not satisfy this reduction. A temporal feature is $z_{icw}=|\mathcal T_w|^{-1}\sum_{k\in\mathcal T_w}X_i[c,k]$, with units of volts.
#
...
#
# **Independent exercise:** Count the highlighted row and column. Why are two of twelve groups targets in one idealized repetition?
...
#
# **Independent exercise:** Calculate target prevalence and the accuracy of always predicting non-target in each session. Do this from labels rather than assuming the idealized 1/6 rate.
#
...
#
# **Independent exercise:** Give two explanations for a small target ERP that do not imply the brain produced no response.
#
# **My reasoning / hand calculation:** _write here._
...
#
# **Independent exercise:** For the first trial and first channel, compute the first window mean. Find the same value in features.
#
# **My reasoning / hand calculation:** _write here._
...
#
# **Independent exercise:** Can the target/non-target feature matrix alone produce a defensible real spelling accuracy? List the missing information.
#
# **My reasoning / hand calculation:** _write here._
